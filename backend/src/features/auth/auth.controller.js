...
const configureSession = (req, rememberMe) => {
  if (rememberMe) {
...
const clearOldAuthCookie = (res) => {
  res.clearCookie("shipit_auth", { path: "/" });
};
...
const loginUser = async (req, res) => {
  try {
    ...
    configureSession(req, rememberMe);
    clearOldAuthCookie(res);
  ...
  try {
    ...
    if (!process.env.EMAIL_USER?.trim() || !process.env.EMAIL_PASS?.trim()) {
      return res.status(500).json({
        message: "Server cannot send email: set EMAIL_USER and EMAIL_PASS in backend/.env (Gmail app password).",
      });
    }
    ...
    try {
      ...
      return res.status(500).json({
        message: mailErr.message || "Failed to send OTP email. Check EMAIL_USER / EMAIL_PASS in backend .env.",
      });
    }
...
const verifyLoginOtp = async (req, res) => {
  try {
    ...
    configureSession(req, Boolean(rememberMe));
    clearOldAuthCookie(res);
...
const startGoogleOAuth = async (req, res) => {
  ...
  if (missing.length) {
    return res.status(500).json({
      message: `Google OAuth not configured. Missing: ${missing.join(", ")}. Set them in backend/.env`,
    });
  }
  const base =
    process.env.FRONTEND_URL || process.env.CLIENT_URL || "http://localhost:5173";
...
const googleOauthCallback = async (req, res) => {
  try {
    const { code, state, rememberMe } = req.query || {};
