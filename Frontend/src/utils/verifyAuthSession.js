  ...
  try {
    ...
    if (status === 401 || status === 403) {
      return {
        ok: false,
        message:
          'Login succeeded but the secure session cookie was not saved. Open the store at your site URL (e.g. http://16.16.115.136/), not :4000. On HTTP, set COOKIE_SECURE=false in backend/.env.',
      };
    }
    return {
      ok: false,
